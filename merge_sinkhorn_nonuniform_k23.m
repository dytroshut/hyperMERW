...
%   out.R2, out.R3         absorbed pivot scalings, Rk=lambda_k*omega_k.*Uk
%   out.K2, out.K3         layer- and pivot-weighted manuscript references
...
%   out.contractionBound   Lemma sufficient-condition coefficient
    ...
    % K^(k)=lambda_k*omega^(k).*A^(k) and
    % R^(k)=lambda_k*omega^(k).*U^(k).
    logU2 = logR2-log(lambda2)-log(omega2);
    logU3 = logR3-log(lambda3)-log(omega3);
    ...
    K2 = lambda2*(A2 .* repmat(omega2,1,n));
    K3 = lambda3*(A3 .* repmat(omega3,1,1,n));
