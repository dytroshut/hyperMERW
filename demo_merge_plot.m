...
codeVersion = 'corrected-power-scaling-v2';
runTimestamp = datetime('now','TimeZone','local');
...
sourceFileInfo = dir(fullfile(scriptDirectory,'*.m'));
...
for k = 1:numberWeights
    y = mixCurves(:,k);
    yplot = max(y,eps);
    h = semilogy(timeSteps,yplot,'LineWidth',1.2);
    idx = 1:markerStep:numel(timeSteps);
    semilogy(timeSteps(idx),yplot(idx),markers{k}, ...
...
figureDirectory = fullfile(scriptDirectory,'figures');
if ~isfolder(figureDirectory)
    mkdir(figureDirectory);
end
outputMatFile = fullfile(scriptDirectory,'merge_mixing_curves_corrected.mat');
outputFigureFile = fullfile(figureDirectory,'merging_corrected.png');

save(outputMatFile, ...
    'codeVersion','runTimestamp','matlabVersion','matlabRelease', ...
    'matlabPlatform','workingDirectory','scriptDirectory','sourceFileInfo', ...
    ...
    'initialError','eta', ...
    'outputMatFile','outputFigureFile');
