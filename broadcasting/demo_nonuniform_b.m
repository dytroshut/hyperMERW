function results = demo_nonuniform_b(outputRoot)
...
for q = 1:size(W,1)
    ...
    assert(out.converged,'Broadcasting solver did not converge at weight row %d.',q);
...
lgd.Box = 'off';
lgd.FontSize = 14;
exportgraphics(gcf,fullfile(figureDirectory,'broadcasting_corrected.png'),'Resolution',300);
...
results.tol = 1e-13;
results.scalarTol = 1e-15;
save(fullfile(resultDirectory,'broadcast_results_corrected.mat'),'results');
end
